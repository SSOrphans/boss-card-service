...
class CardControllerTest
{
	...
	void test_CanGetAllCards() throws Exception
	{
		...
		mockResponse = mvc.perform(get("/api/cards")).andReturn().getResponse();
	...
	@Test
	void test_CanGetAllCards_NotFound() throws Exception
	{
		when(cardService.findAllCards()).thenThrow(new NotFoundException("test"));
		mockResponse = mvc.perform(get("/api/cards")).andReturn().getResponse();
	...
	void test_CanAddCard() throws Exception
	{
		...
		mockResponse = mvc.perform(post("/api/cards").contentType(MediaType.APPLICATION_JSON)
				.content(mapper.writeValueAsString(cardA.convertToCardDto()))).andReturn().getResponse();
